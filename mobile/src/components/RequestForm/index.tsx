...
type Props={initial?:RequestPayload;submitLabel:string;onSubmit(input:RequestPayload):Promise<void>};
export function RequestForm({initial,submitLabel,onSubmit}:Props){
 ...
 return <View style={styles.form}><AppInput label="Título" value={title} onChangeText={setTitle} maxLength={120}/><AppInput label="Descrição" value={description} onChangeText={setDescription} multiline numberOfLines={5} maxLength={2000} style={styles.description}/>
  ...
  {loadingCategories?<Text style={styles.help}>Carregando categorias...</Text>:<ChoiceChips label="Categoria" value={categoryId} onChange={setCategoryId} options={categories.map(item=>({value:item.id,label:item.name}))}/>}{!!error&&<Text accessibilityRole="alert" style={styles.error}>{error}</Text>}<AppButton title={submitLabel} onPress={()=>void submit()} loading={loading} disabled={loadingCategories}/></View>;
}
